      ...
      <Group>
        <Group.List>
         <HBox key={data.id} className="cust-link-list" vAlign="center">
            <VBox  className="t-FBV t-FB1 t-PL20 cust-link-content-list" onClick={ t.clickItem.bind(t, data.custId, data.id, 'get') } >
              <HBox className="item-left t-MT6">姓名：{ data.linkName }</HBox>
              <HBox className="item-left t-MT6">电话：{ data.linkTele }</HBox>
              <HBox className="item-left t-MT6">邮箱：{ data.linkEmail }</HBox>
            </VBox>
